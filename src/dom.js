export const infoSectionTitle = document.querySelector(".info-section-title");
export const totalAidReceived = document.querySelector(".total-aid-received");
export const totalAidAmount = document.querySelector(".total-aid-amount");
export const aidPercentages = document.querySelector(".aid-percentages");
export const aidExplanation = document.querySelector(".aid-explanation");
export const aidStatus = document.querySelector(".aid-status");

export const iconHome = document.querySelector("#icon-home");
export const iconInfo = document.querySelector("#icon-info");
export const whatAndHowTo = document.querySelector(".what-and-how-to");
export const chooseContinent = document.querySelector(".choose-continent");
export const chooseContinentAfrica = document.querySelector(".choose-continent-africa");
export const infoSectionContainer = document.querySelector("#info-section-container");
